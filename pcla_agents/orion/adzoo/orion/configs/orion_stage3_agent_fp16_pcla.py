# PCLA config for the ORION closed-loop agent.
#
# Base: `orion_stage3_agent.py` -- the only inference config that defines
# `inference_only_pipeline`, which the CARLA agent requires. (`orion_stage3_fp16.py`
# is for offline evaluation and lacks it.)
#
# Two edits on top of that base:
#   1. `llm_path` -> PCLA's checkpoint folder instead of ./ckpts.
#   2. fp16 inference enabled, copied from `orion_stage3_fp16.py`: ORION is ~7.5B
#      params, so fp32 would need ~30 GB and will not fit a 20 GB card. With fp16 the
#      model lands at ~16 GB. Do NOT remove this on a 20 GB GPU.
#
# `flash_attn` is left True (upstream's setting; flash-attn is installed -- see
# environment.yml).
...
use_memory = True
# PCLA: upstream's agent config runs fp32. The detector asserts fp16 and fp32
# inference are mutually exclusive (`assert not fp16_infer if fp32_infer else True`),
# and fp32 would need ~30 GB for this ~7.5B model. Disabled in favour of fp16.
fp32_infer=False
num_gpus = 32
batch_size = 4
num_iters_per_epoch = 234769 // (num_gpus * batch_size)
num_epochs = 6
llm_path = './pcla_agents/orion_pretrained/pretrain_qformer/'
use_gen_token = True
use_col_loss = True
...
fp16_infer=True  # PCLA: required to fit ~7.5B params on a 20 GB GPU

model = dict(
    type='Orion',
    fp16_infer=fp16_infer,
    # PCLA: MUST stay False for closed-loop driving. `fp16_eval` selects the offline
    # benchmarking path, which is gated in the detector as:
    #     if not (fp16_infer or fp32_infer) or fp16_eval:
    #         gt_attr_label = data['gt_attr_labels'][0] ...
    # i.e. setting it True forces the branch that reads ground-truth boxes/labels.
    # Those exist only when replaying an annotated dataset; in CARLA there is no
    # ground truth in `data`, so it raises KeyError: 'gt_attr_labels' on frame 0.
    # (`orion_stage3_fp16.py` sets fp16_eval=fp16_infer because it *is* the offline
    # evaluation config -- that pairing must not be carried over to the agent.)
    fp16_eval=False,
