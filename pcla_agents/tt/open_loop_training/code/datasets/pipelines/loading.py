...
# PCLA tt port: these loaders are training-only. The agent supplies camera images and lidar
# points directly and excludes LoadMultiImages/LoadPoints/LoadDepth/LoadSeg from the test
# pipeline, so this module only needs to import (its transforms are never instantiated).
from open_loop_training.code.tt_data import (DataContainer as DC,
                                             TT_TRANSFORMS as PIPELINES)
try:
    import mmcv  # noqa: F401  (only used by the unused disk/ceph loaders at runtime)
except Exception:  # pragma: no cover
    mmcv = None


class LoadPointsFromFile(object):  # stub base for the unused LoadPoints loader
    def __init__(self, *args, **kwargs):
        raise RuntimeError(
            'LoadPointsFromFile is training-only and unavailable in the PCLA tt port')
