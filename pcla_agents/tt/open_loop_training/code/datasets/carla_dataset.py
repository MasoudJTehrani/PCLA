...
import math
import io
# PCLA tt port: the agent only uses union2one / get_ego_shift from this module. The training
# CarlaDataset class is kept for import compatibility but is never instantiated at inference,
# so its heavy training-only bases are stubbed out.
from open_loop_training.code.tt_data import DataContainer as DC, Compose  # noqa: F401
...
DATASETS = _NoopRegistry()
CustomDataset = object
BaseDataset = object  # training-only base; CarlaDataset is not instantiated at inference
try:
    from nuscenes.eval.common.utils import Quaternion  # noqa: F401
