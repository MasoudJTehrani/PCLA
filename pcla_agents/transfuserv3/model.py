...
# Implement bias_init_with_prob if not available
def bias_init_with_prob(prior_prob=0.01):
    """Initialize conv/fc bias value according to a given probability value."""
    bias_init = float(-np.log((1 - prior_prob) / prior_prob))
    return bias_init

# Implement normal_init replacement
...
# Replace force_fp32 with a compatible version
def force_fp32(apply_to=None, out_fp16=False):
    """Decorator to convert inputs to fp32 in forward method."""
    def decorator(func):
...
class LidarCenterNetHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 init_cfg=None):
        super(LidarCenterNetHead, self).__init__(init_cfg=init_cfg)
        ...
        # Build losses - mmdet 3.x uses a different loss building mechanism
        self.loss_center_heatmap = self._build_loss(loss_center_heatmap)
        self.loss_wh = self._build_loss(loss_wh)
        self.loss_offset = self._build_loss(loss_offset)
        self.loss_dir_class = self._build_loss(loss_dir_class)
        self.loss_dir_res = self._build_loss(loss_dir_res)
        self.loss_velocity = self._build_loss(loss_velocity)
        self.loss_brake = self._build_loss(loss_brake)

        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self.fp16_enabled = getattr(train_cfg, 'fp16_enabled', False)
        self.i = 0
    ...
    def _build_loss(self, loss_cfg):
        ...
        if loss_type == 'GaussianFocalLoss':
            return GaussianFocalLoss(**loss_cfg)
            ...
            # Fallback to MODELS registry if available
            try:
    ...
    def init_weights(self):
        ...
        bias_init = bias_init_with_prob(
            self.train_cfg.center_net_bias_init_with_prob if hasattr(self.train_cfg, 'center_net_bias_init_with_prob') else 0.01
        )
        # Use nn.init directly instead of mmcv utilities
        nn.init.constant_(self.heatmap_head[-1].bias, bias_init)
        
        for head in [self.wh_head, self.offset_head]:
            for m in head.modules():
                if isinstance(m, nn.Conv2d):
                    std = self.train_cfg.center_net_normal_init_std if hasattr(self.train_cfg, 'center_net_normal_init_std') else 0.001
