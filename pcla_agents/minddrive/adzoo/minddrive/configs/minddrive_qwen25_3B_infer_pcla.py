# PCLA port of `minddrive_qwen25_3B_infer.py`. Same three edits as the 0.5B variant:
#   1. `llm_path` -> PCLA's checkpoint folder;
#   2. `flash_attn` True (upstream's setting; requires flash-attn -- see
#      environment.yml). Set False for the equivalent vanilla fallback.
#   3. `load_from` cleared - PCLA passes the checkpoint via the agent conf string.
...
num_epochs = 6
llm_path = './pcla_agents/minddrive_pretrained/llava-qwen2.5-3b'
use_gen_token = True
use_col_loss = True
...
model = dict(
    type='Minddrive',
    output_text = False,
    save_path='./work_dirs/text/',  #save path for vlm models.
    use_grid_mask=True,
    frozen=False,
    use_lora=True,
    full_train = False,
...
train_pipeline = [
    ...
    dict(type='LoadAnnoatationVQA', 
        base_desc_path='./data/chat-b2d-plus/train',
        tokenizer=llm_path, 
        max_length=2048, 
    ...
    dict(type='CustomCollect3D',\
         keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'ego_his_trajs','input_ids','gt_attr_labels', 'ego_fut_trajs', 'ego_fut_masks','ego_fut_cmd', 'ego_lcf_feat','vlm_labels','can_bus', 'traffic_state_mask', 'path_points_future','path_future_mask', 'traffic_state','vlm_attn_mask','cmd_speed','cmd_path']+collect_keys),
]
...
load_from = None  # PCLA supplies the checkpoint via the conf string
resume_from=None