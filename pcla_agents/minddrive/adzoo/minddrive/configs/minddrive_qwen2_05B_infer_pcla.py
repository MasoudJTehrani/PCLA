# PCLA port of `minddrive_qwen2_05B_infer.py`. Differences from upstream:
#   1. `llm_path` points at PCLA's checkpoint folder instead of ./Bench2DriveZoo/ckpts.
#   2. `flash_attn` is True (upstream's setting, and how the published scores were
#      measured). Requires flash-attn in the env -- see environment.yml. If it is
#      ever unavailable, set these to False for the mathematically equivalent
#      vanilla-attention fallback (slower, more VRAM).
#   3. `load_from` is cleared - PCLA passes the checkpoint explicitly via the
#      agent's "<config>+<checkpoint>" conf string, so it must not be auto-loaded here.
# Regenerate by re-applying these three edits if upstream changes.
...
num_epochs = 10
llm_path = './pcla_agents/minddrive_pretrained/llava-qwen2-0.5b'
