...
# PCLA port: flash-attn is an optional accelerator here, not a requirement. Every
# attention site in this model (EVAViT, PETRTemporalTransformer, DREAMTransformer-
# Decoder) has an `if self.flash_attn: ... else: ...` branch whose fallback is
# mathematically equivalent vanilla attention, so the model runs correctly without
# it -- just slower. Upstream imports flash_attn at module scope, which would make
# this module unimportable in PCLA's env (flash-attn is not installed). Degrade to
# a clear runtime error instead of an import-time crash, so configs that set
# flash_attn=False import and run cleanly.
...
try:
    ...
    def _flash_attn_unavailable(*args, **kwargs):
        raise ImportError(
            'flash-attn is not installed, but this model was built with '
            'flash_attn=True. Either `pip install flash-attn`, or set '
            'flash_attn=False in the config to use the (slower) vanilla '
            'attention fallback.')

    flash_attn_varlen_kvpacked_func = _flash_attn_unavailable
    unpad_input = pad_input = index_first_axis = _flash_attn_unavailable
