...
# PCLA port: `nms_gpu` lives in the compiled `iou3d_cuda` extension, which this
# port deliberately does not build (no nvcc). Both of its call sites below are
# gated behind `os.getenv('DEBUG_SHOW_DIR_PRED')`, so it is unreachable during
# normal closed-loop inference. Import it lazily so the absence of the compiled
# op does not make this module unimportable.
try:
    from mmcv.ops.iou3d_det import nms_gpu
except (ImportError, ModuleNotFoundError):  # pragma: no cover - depends on build
    def nms_gpu(*args, **kwargs):
        raise ImportError(
            'nms_gpu requires the compiled iou3d_cuda extension, which is not '
            'built in the PCLA port. It is only reachable via the '
            'DEBUG_SHOW_DIR_PRED debug path; unset that variable for normal '
            'inference.')
