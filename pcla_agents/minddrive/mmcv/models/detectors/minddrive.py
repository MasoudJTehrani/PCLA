...
# PCLA port: upstream had three stray editor auto-imports here --
#   `from sqlite3 import Timestamp`, `from black import out`, and
#   `from zmq import PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_MESSAGE`.
# None of the three symbols is referenced anywhere in this file, but they made the
# detector require `black` and `pyzmq` at import time. Removed rather than
# installing formatter/messaging packages to satisfy dead imports.
import torch
...
# PCLA port: upstream took `CategoricalDistribution` from stable-baselines3, whose
# install would risk perturbing the shared PCLA env (gymnasium / pandas / matplotlib).
# It is a thin wrapper over torch.distributions.Categorical, reproduced locally.
from mmcv.models.utils.sb3_distributions import CategoricalDistribution
import time
