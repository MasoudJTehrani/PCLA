...
# PCLA port: `iou3d_cuda` is a compiled extension this inference-only port does not
# build (no nvcc). Its consumers here (nms_gpu / boxes_iou_bev) are only reached via
# the DEBUG_SHOW_DIR_PRED debug path in the detection head, never during normal
# closed-loop inference. Defer the failure from import time to call time.
try:
    from . import iou3d_cuda
except (ImportError, ModuleNotFoundError, OSError):  # pragma: no cover
    from mmcv.utils.ext_loader import missing_ext
    iou3d_cuda = missing_ext('iou3d_cuda')
