...
class ObsManager(ObsManagerBase):
  ...
  def _warp_hd_map(self, m_warp):
    ...
    output image, whose size is self.width x self.width.

    Computes which source pixels are needed for the current local BEV frame,
    crops that region, adjusts the affine transform to the crop coordinate
    system, and then calls cv.warpAffine() on the crop.
    """

    # self.hd_map_array has shape (height, width, channels).
    map_height, map_width = self.hd_map_array.shape[:2]

    # If the full map is small enough, keep exactly the original behavior.
    if map_height < 32767 and map_width < 32767:
      return cv.warpAffine(self.hd_map_array, m_warp, (self.width, self.width))
    ...
    if x_min >= x_max or y_min >= y_max:
      channels = self.hd_map_array.shape[2]
      return np.zeros((self.width, self.width, channels), dtype=np.uint8)
    ...
    if crop_height >= 32767 or crop_width >= 32767:
      raise RuntimeError(
          f"CaRL HD map crop is still too large for OpenCV: "
    ...
    return cv.warpAffine(hd_map_crop, crop_warp, (self.width, self.width))
