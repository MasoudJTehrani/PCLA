...
class BaseAgent:
    ...
    def setup(self, sensor_agent: bool = False):
        ...
        # Route planners and the GPS reference consume the global plan, which
        # only exists after `set_global_plan`. They are initialised lazily in
        # `init_route_planners`, invoked from each agent's deferred `_init`
        # (first step) — not here, since `setup` runs at construction time.
        self.sensor_agent = sensor_agent

    @beartype
    def init_route_planners(self):
        """Initialise the GPS reference and route planners from the global plan.

        Must run after `set_global_plan` has populated the stored plan, so it is
        called from each agent's deferred `_init`, not from `setup` (which runs
        at construction, before the plan is known).
        """
